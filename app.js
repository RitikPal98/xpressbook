...
mongoose.connect("mongodb+srv://admin-ritik:mdbritikpal@cluster0.qvhbv.mongodb.net/ExpressBook",{useNewUrlParser: true, useUnifiedTopology: true});
...
passport.use(new GoogleStrategy({
    ...
    callbackURL: "https://xpressbook.herokuapp.com/auth/google/secret"
  },
  function(accessToken, refreshToken, profile, cb) {
    // console.log(profile);

    User.findOrCreate({fullName:profile.displayName,photo:profile._json.picture,username:profile.id,googleId: profile.id }, function (err, user) {
      return cb(err, user);
    });
  }
));
    ...
    User.findOne({_id:req.user.id},function(err,currentUser){
      ...
      currentUser.following.forEach(function(followee){
        ...
        User.findOne({username:followee},function(error,foundFollowee){
          MyPost.find({postBy:foundFollowee._id},function(notFound,followeePosts){
            followeePosts.forEach(function(singlePost){

              allposts.push({followeePost:singlePost,
...
app.post("/signup",function(req,res){
  ...
  User.register({
    ...
    userBio:"Hello, It's me."}, req.body.password, function(err, user){
    if (err) {
      console.log(err);res.redirect("/user/LogIN");
    } else {
...
userphoto=req.user.photo;
notify=req.user.notify;
}else{userphoto="/avengersicon.png"}
...
if(post==null){res.send("<h1>This post has been deleted</h1>");}else{
if(!err){
if(req.user==null || post==null){likeAndDislike="/likes";}else{
...
if(req.user==null){followAndunfollow="Follow";}else{
if(post.postBy===req.user.id){followAndunfollow="You";}else{
postuser.followers.forEach(function(followee){
if(followee===req.user.username){followAndunfollow="Unfollow";}else{
...
MyPost.findByIdAndRemove(postId,function(err){
    if(!err){console.log("succefully Deleted the item");
    res.redirect("/profile");}
  });
});
...
MyPost.updateOne({_id:req.body.postId},{$push:{comments:commentone
}
},function(err,done){
  if(err){console.log(err);}else{console.log("Added the comment");
}
});
...
followBtn="Find";
  ...
  followBtn="Follow"}
    ...
    User.findOne({username:req.params.username},function(err,postUser){
      postUser.followers.forEach(function(followee){
      if(followee===req.user.username){followRedirectUrl="Unfollow";
    followBtn="Unfollow";}
      });
...
MyPost.updateOne({_id:req.body.postId},{$push:{likes:req.user.username}
},function(err,done){
  if(err){console.log(err)}else{console.log("like Successful");
let redirectUrl="posts/"+req.body.postId;
...
if(req.isAuthenticated()){
MyPost.updateOne({_id:req.body.postId},{$pull:{likes:req.user.username}
},function(err,done){
  if(err){console.log(err)}else{console.log("Dislike Successful");
// let redirectUrl="posts/"+req.body.postId;
// res.redirect("/notifi/"+redirectUrl+"/likepage/like");
// res.redirect("/posts/"+req.body.postId);
}});
...
if(theuser.followers.length===0){
  res.render("people",{users:[],userphoto:req.user.photo,notify:req.user.notify});
}else{
        ...
        User.findOne({username:follower},function(err,thefollower){
          ...
          if(i===theuser.followers.length){
            res.render("people",{users:allthefollowers,userphoto:req.user.photo,notify:req.user.notify});          }
        });});}
      });}else if(req.params.all==="following"){
          User.findOne({_id:req.params.user},function(err,theuser){
if(theuser.following.length===0){res.render("people",{users:[],userphoto:req.user.photo,notify:req.user.notify});
}else{
...
if(req.body.postId===req.body.postuser){redirectUrl=req.body.postuser+"/profile"}else{
redirectUrl="posts/"+req.body.postId;}

User.updateOne({username:req.body.postuser},{$push:{followers:req.user.username
}
},function(err,done){
    if(err){console.log(err)}else{console.log("new follower added Successfully");}});
//Adding user to following
User.updateOne({username:req.user.username},{$push:{following:req.body.postuser}
},function(err,done){ if(err){console.log(err);}else{
  console.log(" Now you are following to",req.body.postuser);
res.redirect("/notifi/"+redirectUrl+"/followpage/"+req.body.postuser);
...
User.updateOne({username:req.body.postuser},{$pull:{followers:req.user.username}
         },function(err,done){
     if(err){console.log(err);}else{console.log("Unfollowed Successfully"); }});
  //deleting user from following
User.updateOne({username:req.user.username},{$pull:{following:req.body.postuser}
},function(error,removed){
  if(error){console.log(err);}else{console.log("Deleted the user from following list");
...
if(result.url===undefined){
  ...
  let photoUrl=result.url.slice(0,58)+"c_thumb,g_center,h_250,w_250/"+result.url.slice(58);

//send photo url string to our database
User.updateOne({_id:req.user.id},{photo:photoUrl},function(err,updated){
  if(!err){console.log("succefully updated");
res.redirect("/profile");}
});}
});}else{
  res.redirect("/user/LogIn");
...
app.post("/remove",function(req,res){
  if(req.isAuthenticated()){
  User.updateOne({_id:req.user.id},{photo:"/avengersicon.png"},function(err,removed){
    if(removed){res.redirect("/edit");}
  });}});
...
if(req.user.username===req.body.username){start="YES";}
if(!foundUser){start="YES";}
if(start==="YES"){
  User.updateOne({_id:req.user.id},{
    ...
    if(!err){console.log("succefully updated");
  res.redirect("/profile");}});}else{res.send("<h4 style='color:red'>Please choose another username.This username has already taken.</h4>")}
});
}else{res.redirect("/user/LogIn");}
});
  ...
  User.findOne({_id:req.user.id},function(err,foundUser){
    if(!err){
      if(foundUser.followers.length===0){
        res.redirect("/profile");}else{

    foundUser.followers.forEach(function(follower){
      User.updateOne({username:follower},{$push:{notifications:{
      ...
      User.updateOne({username:follower},{notify:"yes"},function(er,dn){
...
MyPost.findOne({_id:req.params.for},function(err,foundPost){
  ...
  User.updateOne({_id:foundPost.postBy},{$push:{notifications:{
    from:by,
    photo:req.user.photo,
    what:message,
    ...
    if(done){
      ...
      //Notify the user
      User.updateOne({_id:foundPost.postBy},{notify:"yes"},function(er,notified){
...
User.updateOne({username:req.params.nameofuser},{$push:{notifications:{
    from:req.user.username,
    photo:req.user.photo,
    what:message,
    ...
    if(done){
      //Notify the user
      User.updateOne({username:req.params.nameofuser},{notify:"yes"},function(er,notified){
        // if(notified){console.log("yup Notify");}
      });
...
app.post("/clearNotifications",function(req,res){
  if(req.isAuthenticated()){
    User.updateOne({_id:req.user.id},{notifications:[]},function(err,cleared){
      if(cleared){
